...
ltn::SwapChain::SwapChain(CoreInstance& core_instance, int w, int h)
	...
	VkExtent2D  extent = choose_swap_extent(m_support_details.capabilities, w, h); 
...
void ltn::SwapChain::create_syncobjects()
{
...
void ltn::SwapChain::create_depth_buffer()
{
...
void ltn::SwapChain::cleanup()
{
	vkDestroySwapchainKHR(this->m_core_instance.get_device(),this->m_swapchain,nullptr );
	for (int i = 0; i < m_imageAvailableSemaphores.size();i++) {
		vkDestroySemaphore(this->m_core_instance.get_device() , m_imageAvailableSemaphores[i], nullptr);
	}
	for (int i = 0; i < m_renderFinishedSemaphores.size(); i++) {
		vkDestroySemaphore(this->m_core_instance.get_device(), m_renderFinishedSemaphores[i], nullptr);	
	}
	for (int i = 0; i < m_inFlightFences.size(); i++) {
		vkDestroyFence(this->m_core_instance.get_device() , m_inFlightFences[i], nullptr);
	}
	for (auto imageView : m_swapChain_image_views) {
		//vkDestroyImage(this->m_core_instance.get_device(), m_swapChain_images[i],nullptr);
		vkDestroyImageView(this->m_core_instance.get_device() , imageView,nullptr);
	}
	
}

// fill in data in SwapChainSupportDetails
void ltn::SwapChain::query_support()
{
...
VkSurfaceFormatKHR ltn::SwapChain::choose_swap_surface_format(const std::vector<VkSurfaceFormatKHR>& availableFormats)
{
...
VkPresentModeKHR ltn::SwapChain::choose_swap_present_mode(const std::vector<VkPresentModeKHR>& availablePresentModes)
{
...
VkExtent2D ltn::SwapChain::choose_swap_extent(const VkSurfaceCapabilitiesKHR& capabilities, const unsigned int w, const unsigned int h)
{
	VkExtent2D actualExtent = {
			w,
			h
	};
	// In case of super-resolution device.
